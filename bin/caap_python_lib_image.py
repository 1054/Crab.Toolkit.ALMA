...
def elliptical_Photometry(image, ellipse=Ellipse([0,0],0,0,0), imagewcs=[], verbose=True):
    ...
    #print(numpy.sum(mask_x*image*mask_pix, axis=1) / numpy.sum(image*mask_pix, axis=1))
    #print(numpy.sum(mask_y*image*mask_pix, axis=0) / numpy.sum(image*mask_pix, axis=0))
    cpix_x = numpy.nanmedian(numpy.sum(mask_x*image*mask_pix, axis=1) / numpy.sum(image*mask_pix, axis=1)) # sum(axis=1) should be summing image X rows for each Y
    cpix_y = numpy.nanmedian(numpy.sum(mask_y*image*mask_pix, axis=0) / numpy.sum(image*mask_pix, axis=0)) # sum(axis=0) should be summing image Y cols for each X
