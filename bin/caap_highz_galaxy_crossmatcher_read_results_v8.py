...
for band in list_band: 
    ...
    for i in range(len(list_Obj)):
        ...
        temp_Position = [numpy.nan, numpy.nan]
        temp_Centroid = [numpy.nan, numpy.nan]
        temp_Source_to_RefSource = numpy.nan
        ...
        if os.path.isfile(temp_Score_File):
            with open(temp_Score_File, 'r') as fp:
                for lp in fp:
                    if lp.startswith('Match.Score'):
                    ...
                    elif lp.startswith('Match.Morphology.Separation'):
                        temp_Sep_Counterpart = (lp.split('=')[1]).split('#')[0]
                    elif lp.startswith('Match.Morphology.Angle'):
                        ...
                        if temp_String.find('nan') < 0:
                            ...
                            print('Found nan in "%s"!'%(temp_Score_File))
                    # 
                    elif lp.startswith('Match.Photometry.Source/RefSource'):
                        temp_Source_to_RefSource = (lp.split('=')[1]).split('#')[0]
                    # 
                fp.close()
        else:
        ...
        list_Source_to_RefSource.append(temp_Source_to_RefSource)
    ...
    with open(output_Txt, 'w') as fp:
        # print header
        fmt = '# {:<10s} {:<%ds} {:>12s} {:>12s} {:>12s} {:>12s} {:>12s} {:>12s} {:>12s} {:>12s} {:>12s} {:>12s} {:>15s} {:>15s} {:>15s}\n'%(strlen_Obj)
        fp.write( fmt.format("INDEX", "OBJECT", "SUBID_TILE", "SNR_FIT", "Separation", "PHOTOZ", "Score_Total", "Score_Morph", "Score_Photo", "Score_Exten", "Crowdedness", "Clean_Index", "Source_to_Ref", "Sep_Centroid", "Ang_Centroid") )
        fp.write( "# \n" )
        # loop each source
        for i in range(len(list_Obj)):
            fmt = '  {:<10d} {:<%ds} {:12d} {:12.3f} {:12.3f} {:12.4f} {:12g} {:12g} {:12g} {:12g} {:12g} {:12g} {:15g} {:15g} {:15g}\n'%(strlen_Obj)
            fp.write( fmt.format( i, list_Obj[i], list_Sid[i], list_SNR[i], list_Sep[i], list_z[i], \
                                  ...
                                  float(list_Source_to_RefSource[i]), float(list_Sep_Centroid[i]), float(list_Ang_Centroid[i]), \
                                )
                    )
        fp.close()
