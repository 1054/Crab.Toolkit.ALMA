    ...
    def match_morphology(self, OutputDir='results', Overwrite=False, FoV=15.0):
        if len(self.FitsImageData)>0 and self.Source and self.RefSource:
            ...
            if True:
                ...
                PlotPanel.coords[0].set_major_formatter('dd:mm:ss.s')
                ...
                #                                    ) * 100.0
                # 
                self.Match['Photometry']['Score'] = ( 0.25 * numpy.min( [ self.Match['Photometry']['S/N']/15.0, 1.0] ) + 
                                                      0.75 * self.Source.Photometry['ALMA Band 6 240 GHz S/N']/15.0
                                                    ) * 100.0
                # 
                # 
                # 
...
for i in range(len(Cat.TableData)):
    ...
    Source = Highz_Galaxy(
        ...
        Redshifts = {
            'Laigle 2015 photo-z': float(Cat.TableData[i].field('ZPDF'))
        }, 
    )
    Source.about()
