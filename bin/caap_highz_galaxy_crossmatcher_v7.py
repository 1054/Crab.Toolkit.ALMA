    ...
    def match_morphology(self, OutputDir='results', OutputName='', Overwrite=False, FoV=15.0):
        ...
        if self.Source and self.RefSource and self.RefImage:
            ...
            if True:
                ...
                # 
                # 
                #<DEBUG><20170428># major = self.Source.Morphology['Major Axis'] / self.RefImage.PixScale[0] # in unit of pixel
                #<DEBUG><20170428># minor = self.Source.Morphology['Minor Axis'] / self.RefImage.PixScale[1] # in unit of pixel
                #<DEBUG><20170428># angle = self.Source.Morphology['Pos Angle']
                #<DEBUG><20170428># zoomposxy = self.RefImage.sky2xy(self.Source.RA, self.Source.Dec)
                #<DEBUG><20170428># zoomellip = Ellipse(xy=zoomposxy, width=major, height=minor, angle=angle, edgecolor=hex2color('#00CC00'), facecolor="none", linewidth=2, zorder=10)
                #<DEBUG><20170428># print(zoomellip)
                #<DEBUG><20170428># print(self.RefImage.get_aperture_by_label('source position 1.00xFWHM')['Ellipse'])
                # 
                ...
                # 
                # 
                # print photometry, calculate flux ratio at source position and refsource position
                # and calculate surface brightness radial profile
                ...
                source_aperture_previous_diameter = None
                for diameter in diameters:
                    source_aperture = self.RefImage.get_aperture_by_label('source position %0.2fxFWHM'%(diameter))
                    refsource_aperture = self.RefImage.get_aperture_by_label('refsource position %0.2fxFWHM'%(diameter))
                    print("Integrated            flux at    source position within %0.2fxFWHM is %15g +- %15g (weighted centroid %.10f %.10f)"%(diameter,    source_aperture['Flux'],                                     source_aperture['Noise'],    source_aperture['Centroid'][0],    source_aperture['Centroid'][1]))
                    print("Background-subtracted flux at    source position within %0.2fxFWHM is %15g +- %15g (weighted centroid %.10f %.10f)"%(diameter,    source_aperture['Flux']-   source_aperture['Background'],    source_aperture['Noise'],    source_aperture['Centroid'][0],    source_aperture['Centroid'][1]))
                    print("Integrated            flux at refsource position within %0.2fxFWHM is %15g +- %15g (weighted centroid %.10f %.10f)"%(diameter, refsource_aperture['Flux'],                                  refsource_aperture['Noise'], refsource_aperture['Centroid'][0], refsource_aperture['Centroid'][1]))
                    print("Background-subtracted flux at refsource position within %0.2fxFWHM is %15g +- %15g (weighted centroid %.10f %.10f)"%(diameter, refsource_aperture['Flux']-refsource_aperture['Background'], refsource_aperture['Noise'], refsource_aperture['Centroid'][0], refsource_aperture['Centroid'][1]))
                    # 
                    if '%0.2fxFWHM'%(diameter) == '1.00xFWHM':
                    ...
                    self.Photometry['EnclosedPower'].append( {
                        'x': diameter, 
                        'y': (source_aperture['Flux']-source_aperture['Background']), 
                        'err': (source_aperture['Noise']), 
                        'area': (source_aperture['Area'])
                    } )
                    # 
                    if diameter == numpy.min(diameters):
                        self.Photometry['GrowthCurve'].append( {
                            'x': diameter, 
                            'y': (source_aperture['Flux']-source_aperture['Background']) / source_aperture['Area'], 
                            'err': (source_aperture['Noise']) / source_aperture['Area'], 
                            'area': (source_aperture['Area'])
                        } )
                    elif source_aperture_previous_diameter:
                        self.Photometry['GrowthCurve'].append( {
                            'x': diameter, 
                            'y': ( (source_aperture['Flux']-source_aperture['Background']) - (source_aperture_previous_diameter['Flux']-source_aperture_previous_diameter['Background']) ) /  (source_aperture['Area'] - source_aperture_previous_diameter['Area']), 
                            'err': ( source_aperture['Noise'] / numpy.sqrt(source_aperture['Pixel Number']) * numpy.sqrt(source_aperture['Pixel Number']-source_aperture_previous_diameter['Pixel Number']) ) /  (source_aperture['Area'] - source_aperture_previous_diameter['Area']), 
                            'area': ( source_aperture['Area'] - source_aperture_previous_diameter['Area'] )
                        } )
                    # 
                    source_aperture_previous_diameter = source_aperture
                # 
                # print photometry S/N info
                for dia in range(len(diameters)):
                    print('Source image aperture with diameter %0.2fxFWHM has flux = %0.6g +- %0.6g and S/N = %0.3f (area = %0.6g)'%(self.Photometry['EnclosedPower'][dia]['x'], self.Photometry['EnclosedPower'][dia]['y'], self.Photometry['EnclosedPower'][dia]['err'], self.Photometry['EnclosedPower'][dia]['y']/self.Photometry['EnclosedPower'][dia]['err'], self.Photometry['EnclosedPower'][dia]['area']))
                # 
                ...
                for dia in range(len(diameters)):
                    ...
                    # convert surface brightness annulus diameter from NxFWHM to arcsec
                    self.Photometry['GrowthCurve'][dia]['x'] = self.Photometry['GrowthCurve'][dia]['x'] * self.Source.Morphology['Major Axis']
                    self.Photometry['GrowthCurve'][dia]['y'] = self.Photometry['GrowthCurve'][dia]['y'] / surface_brightness_source
                    self.Photometry['GrowthCurve'][dia]['err'] = self.Photometry['GrowthCurve'][dia]['err'] / surface_brightness_source
                # 
                # 
                # 
                # 
                ...
                if signal_to_noise_ratio_source >= 5.0:
                    ...
                    #print(fitting_coeff_inner)
                    #print(fitting_coeff_outer)
                    #print(fitting_coeff_middle)
                    #print(fitting_coeff_overall)
                    source_extent['Inner slope'] = fitting_coeff_inner[-1]
                    source_extent['Outer slope'] = fitting_coeff_outer[-1]
                    source_extent['Middle slope'] = fitting_coeff_middle[-1]
                    source_extent['Overall slope'] = fitting_coeff_overall[-1]
                    print('Source surface brightness radial profile Inner slope = %0.3f'%(source_extent['Inner slope']))
                    print('Source surface brightness radial profile Outer slope = %0.3f'%(source_extent['Outer slope']))
                    print('Source surface brightness radial profile Middle slope = %0.3f'%(source_extent['Middle slope']))
                    print('Source surface brightness radial profile Overall slope = %0.3f'%(source_extent['Overall slope']))
                    # 
                    ...
                    if len(fitting_data_y) <= 1: 
                        ...
                        print('Source surface brightness outer/inner S/N>=3.0 annulus number is not enough to calcualate the source extended parameter!')
                    else:
                        self.Morphology['Extended'] = fitting_data_y[-1] / fitting_data_y[0] * 100.0 # / (fitting_data_x[-1]-fitting_data_x[0])
                        print('Source surface brightness outer/inner S/N>=3.0 annulus ratio = %0.3f (with diameters %0.2f and %0.2f arcsec)'%(fitting_data_y[-1] / fitting_data_y[0], fitting_data_x[-1], fitting_data_x[0]))
                else:
                    print('The source S/N is lower than 5.0 (undetected), we will not calcuate the source mophological extension parameter')
                # 
                # 
                ...
                if self.Morphology['Extended'] > 0 and self.Morphology['Extended'] == self.Morphology['Extended']:
                    if self.Photometry['S/N'] >= 5.0:
                        if self.Morphology['Extended'] > 100.0:
                            offset_down_weighting = numpy.min([numpy.max([self.Morphology['Extended']/100.0, 1.0]), 3.0]) # -- <20170308> only down-weight if source image S/N>5.0
                ...
                self.Morphology['Score'] = 100.0 * \
                                                    ( 1.0 - 
                                                      1.0 * (
                                                        self.Morphology['Separation'] / offset_down_weighting / (
                ...
                # 
                # 
                self.Photometry['Score'] = numpy.min( [ self.Source.Photometry['ALMA Band 6 240 GHz S/N']/12.0, 1.0 ] ) * 100.0
                ...
                TextFilePtr.write("Match.Morphology.Separation = %s # Source to RefSource separation distance in arcsec\n"%(self.Morphology['Separation']))
                TextFilePtr.write("Match.Morphology.Angle = %s # Source to RefSource (PosAngle - SepAngle) in degree\n"%(self.Morphology['Angle']))
                TextFilePtr.write("Match.Morphology.Extended = %.1f # Source surface brightness radial annuli outer/inner ratio\n"%(self.Morphology['Extended']))
