...
Cat = Highz_Catalogue('Match_cycle2_new_detections_1.5arcsec.fits')
...
for i in range(len(Cat.TableData)):
    ...
    # Copy Cutouts
    CutoutFilePaths = glob.glob("/home/dzliu/aida42198_ALMA_Data/ALMA_COSMOS/cutouts/45arcsec/*/%s[._]*.fits"%(Source.Name))
    CutoutFileNames = []
    if len(CutoutFilePaths)==0:
        print("***********************************************")
        print("Error! Could not find \"/home/dzliu/aida42198_ALMA_Data/ALMA_COSMOS/cutouts/45arcsec/*/%s*.fits\"!"%(Source.Name))
        print("***********************************************")
        #sys.exit()
        continue
    else:
